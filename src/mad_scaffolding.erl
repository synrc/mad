...
tpl(Params) -> 
    ...
    case proplists:get_value(tpl, Vars) of
     undefined -> 
         mad:info("please specify a template: scaffolding tpl=<TemplateName> appid=<name>...~n",[]),true;
     TplName -> 
         AppName = proplists:get_value(appid, Vars),
         mad:info("AppId = ~p~n",[AppName]),
         mad:info("TplName = ~p~n",[TplName]),
         mad:info("Vars = ~p~n",[Vars]),
         case proplists:get_value("priv/" ++ TplName ++ ".skel", Apps) of 
          undefined -> case getskel(TplName) of
                        {error,F} -> mad:info("unable to load the template ~p.~n",[F]),true;
                        Bin       -> TemplateTerms = consult(Bin),
                                     Vars1 = lists:keyreplace(name, 1, Vars, {appid, AppName}),
                                     create({dir,tpldir()}, TemplateTerms, Vars1) 
                       end;

          Skel      -> TemplateTerms = consult(Skel),
                       Vars1 = lists:keyreplace(name, 1, Vars, {appid, AppName}),
                       create({mem,Apps}, TemplateTerms, Vars1)          
         end
    end.

tpldir() -> case os:get_env_var("MAD_TEMPLATE_DIR") of
              false -> filename:join([os:get_env_var("HOME"), ".mad", "templates"]);
              Dir -> Dir end.

getskel(TplName) ->
 F = filename:join([tpldir(),TplName++".skel"]),
 case file:read_file(F) of {error,_} -> {error,F}; {ok,Bin}  -> Bin end.
...
create(Src, FinalTemplate, VarsCtx) ->
    ...
    execute_template(Src, FinalTemplate, none, Name, Context, Force, []),
    {ok,Name}.
...
consult(Source) ->
    ...
    ParseFun = fun (Form) ->
                       ...
                       {ok, Expr} -> Expr;
                       E -> mad:info("Error mad_scaffolding ~p", [E]), E end 
               end,
...
%%
load_file({dir,Dir}, _, Name) ->
    case file:read_file(filename:join(Dir,Name)) of
        {_, Bin} -> Bin;
        _ -> ok
    end;
load_file({mem,Files}, _, Name) ->
    ...
    case Force =:= "1" orelse FileExists =:= false of
        true ->
            ...
            case {Force, FileExists} of
                {"1", true} ->
                    mad:info("Writing ~s (forcibly overwriting)~n",
                             [Output]);
                _ ->
                    mad:info("Writing ~s~n", [Output])
            end,
            case file:write_file(Output, Data) of
                ok ->
                    ok;
                {error, Reason} ->
                    mad:info("Failed to write output file ~p: ~p\n",
                           [Output, Reason])
            end;
        false ->
            {error, exists}
    ...
    case ExistingFiles of
        ...
        _ ->
            ...
            Help = "To force overwriting, specify -f/--force/force=1"
                " on the command line.\n",
            mad:info("One or more files already exist on disk and "
